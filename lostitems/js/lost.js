...
submmit.onclick = function() {
	...
	$.cookie('time',time.value);
	$.cookie('space',fenlei[0].textContent);
	$.cookie('species',fenlei[1].textContent);
	$.cookie('picture',name);
	alert("提交成功")
	window.location.href='http://120.79.86.1:8080/lostitems/LostBar.html'
	/*ajax({
		method: 'POST',
		url: 'http://120.79.86.1:8080/cqupt-items-platom/releaseLost',
		data: {
			lostTime: time.value,
			lostAddress: fenlei[0].textContent,
			species: fenlei[1].textContent,
			description: title.textContent,
			lostItems: pstimg.name
		},
		async: true,
		dataType: 'JSON',
